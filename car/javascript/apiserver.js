...
const { FileSystemWallet, Gateway } = require('network');  
...
app.get('/api/queryallcars', async function (req, res) {  
 	try {
        ...
        const network = await gateway.getNetwork('maycar');

        const contract = network.getContract('car');
...
app.get('/api/query/:car_index', async function (req, res) {  
 	try {
        ...
        const network = await gateway.getNetwork('mycar');
        const contract = network.getContract('car');
        // queryCar transaction - requires 1 argument, ex: ('queryCar', 'CAR4')
        // queryAllCars transaction - requires no arguments, ex: ('queryAllCars')
